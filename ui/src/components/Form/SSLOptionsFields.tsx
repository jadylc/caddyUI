...
import { CertificateAuthorityFields, DNSProviderFields, DomainNamesField } from "src/components";
...
export function SSLOptionsFields({
	...
	const hasCertificate = newCertificate || v?.certificateId === -1 || (v?.certificateId && v?.certificateId > 0);
	const { sslForced, hstsEnabled, hstsSubdomains, trustForwardedProto, meta } = v;
	...
	}, [dnsChallenge, forceDNSForNew, newCertificate, setFieldValue]);
	...
	return (
		<div>
			...
			{newCertificate ? (
				<>
					<CertificateAuthorityFields name="meta.issuerProvider" />
