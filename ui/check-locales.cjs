...
const extractResult = spawnSync(npmCommand, ["run", "locale-extract", "--", "--out-file", tmpobj.name], {
	stdio: "inherit",
	shell: process.platform === "win32",
});
if (extractResult.status !== 0) {
	console.log("\x1b[31m%s\x1b[0m", `ERROR: locale extraction failed${extractResult.error ? `: ${extractResult.error.message}` : ""}`);
